...
class User(Core):
    def __init__(self, url, session, id=None, first_name=None, is_active=None, password=None, repeat_password=None,
                 roles=None,
    ...
    def _check_self_user(self):
        ...
        result_user = self._get_dict_user(username=self.username, domain=self.user_domain)
        if not result_user:
            return False
        else:
    ...
    def get_user(self, username=None, domain=None):
        ...
        for field in result_user.keys():
            if field == 'id':
                setattr(user, 'id', result_user[field])
